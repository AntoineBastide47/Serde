...
struct Vec2 : Serde::Reflectable {
  ...
  [[nodiscard]] std::string_view ClassNameQualified() const override {
    return "Vec2";
  }
...
static void testConstructors() {
  ...
  using namespace Serde; {
    JSON j;
    check(
      j.IsNull() && !j.IsBool() && !j.IsNumber() && !j.IsString() && !j.IsArray() && !j.IsObject(),
      "default ctor → null"
    );
  } {
    JSON t(true), f(false);
  ...
  using namespace Serde; {
    ...
    check(
      ...
      check(
        ...
        ), "const operator[] non-array throws"
      );
    }
    ...
    check(
      throws_as<std::logic_error>(
        [&] {
          JSON("x").Insert(0, JSON(0));
        }
      ), "Insert non-array throws"
    );
  } {
    ...
    check(
      throws_as<std::exception>(
        [&] {
          JSON(42).Contains(JSON(1));
        }
      ), "Contains(JSON) non-array/obj throws"
    );
  }
}
  ...
  using namespace Serde; {
    ...
    check(
      ...
      check(
        ...
        ), "const operator[] non-object throws"
      );
    }
    ...
    check(
      throws_as<std::exception>(
        [&] {
          JSON::Array().Contains("x");
        }
      ), "Contains(key) on array throws"
    );
  } {
...
static void testLoad() {
  ...
  // forward_list round-trip
  {
    std::forward_list<int> fl = {1, 2, 3};
    check(
      Deserializer::FromJson<std::forward_list<int>>(Serializer::ToJson(fl)) == fl,
      "forward_list round-trip"
    );
  }
  // map round-trip
  ...
  // tuple round-trip
  {
    auto t = std::make_tuple(1, true, std::string("z"));
    check(
      Deserializer::FromJson<std::tuple<int, bool, std::string>>(Serializer::ToJson(t)) == t,
      "tuple round-trip"
    );
  }
  // fixed C array
  {
    ...
    check(
      throws_as<std::runtime_error>(
        [&] {
          _e_loadImpl(arr2, Format::JSON, bad);
        }
      ),
  ...
  // variant round-trips
  {
    ...
    check(
      std::holds_alternative<std::string>(v2) && std::get<std::string>(v2) == "world",
  ...
  // enum round-trip
  {
    ...
    check(
      throws_as<std::runtime_error>(
        [&] {
          (void)Deserializer::FromJson<Color>(bad);
        }
      ),
